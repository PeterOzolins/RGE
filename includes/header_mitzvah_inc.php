                        <div id="headerTop">
                            <!-- include actual header menu -->
                            <?php
                                include 'header_menu_inc.php';
                            ?>

                            <div class="clearfix"></div>
                            <div id="headerImage" class="events">
                                <img src="/img/home-top-borderline.png" id="topHeadBorderline">

                                <!--@@@  iFrame for slideshow -->
                                    <iframe src="/includes/slideshow_mitz.html" seamless="seamless" id="slideshowIframe"></iframe>
                                <!--@@@  END iFrame for slideshow -->

                                <img src="/img/home-middle-borderline.png" id="midHeadBorderline">
                            </div>
                        </div>
                        <div id="headerBottom">
                            <div id="headerBottomInner">
                                <div id="headerBottomInnerLeft">
                                    <p>
                                        Like Us!
                                    </p>
                                        <div class="fbBtnInline">
                                            <div class="fb-like" data-href="http://realgeniusdjs.com.s50849.gridserver.com/" data-send="false" data-layout="button_count" data-width="200" data-show-faces="false" data-font="arial"></div>
                                        </div>
                                </div>
                                <div id="headerBottomInnerRight">
                                    <p>
                                        Book Your Talent Today! 1-800-555-9999
                                    </p>
                                </div>
                                <img src="/img/home-bottom-borderline.png" id="bottomHeadBorderline">
                            </div>
                        </div>
